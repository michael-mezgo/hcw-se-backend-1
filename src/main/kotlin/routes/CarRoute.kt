    ...
    routing {
        route("/cars") {
            get({
                ...
                request {
                    queryParameter<Boolean>("available") {
                        description = "If true, only available cars are returned"; required = false
                    }
                    queryParameter<String>("currencyCode") {
                        description =
                            "Currency Code - Supported currencies: ${CurrencyService.getSupportedCurrencies()}"
                    }
                }
                response {
                ...
                call.respond(
                    HttpStatusCode.OK,
                    cars.map { it.toResponse(blobStorageService = blobStorage, currencyCode = currencyCode) })
            }
            ...
            get("/{id}", {
                ...
                call.respond(
                    HttpStatusCode.OK,
                    car.toResponse(blobStorageService = blobStorage, currencyCode = currencyCode)
                )
            }
            ...
            authenticate("user-jwt") {
                post("/{id}/bookings", {
                    tags("Cars")
                    summary = "Book a car"
                    description = "Creates a booking for this car"
                    ...
                    when (carService.book(id, user.id)) {
                        BookingResult.CarNotFound ->
                            throw ServiceException.NotFound("Car not found")

                        BookingResult.UserNotFound ->
                            throw ServiceException.NotFound("User not found")

                        BookingResult.CarUnavailable ->
                            throw ServiceException.Conflict("Car is not available")
            ...
            authenticate("user-jwt") {
                ...
                delete("/{id}/bookings") {
                    ...
                    val principal = call.principal<JwtPrincipal>()
                        ?: throw ServiceException.Unauthorized("Missing token")
                    ...
                    if (car.bookedBy == null) {
                        throw ServiceException.Conflict("Car is not booked")
                    }

                    if (!principal.isAdmin && car.bookedBy.id != principal.userId) {
                    ...
                    when (carService.unbook(id)) {
                        ...
                        BookingResult.CarUnbooked ->
                            call.respond(HttpStatusCode.OK, mapOf("message" to "Car unbooked successfully"))
